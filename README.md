...
Welcome! This is an automated workshop that will explain what [account Abstraction](https://perama-v.github.io/cairo/account-abstraction) is and how you can leverage it to create powerful custom account contracts.
...
TL;DR: accounts on StarkNet are simply regular smart contracts. One caveat is that they MUST have canonical entry point denoted with the selectors:

- `__validate__`
- `__validate_declare__`
- `__execute__`

Your goal is to design account contracts that pass all the [evaluator.cairo](contracts/tutorial/evaluator.cairo) checks and collect all the points available on StarkNet(Goerli).
...
- Add exercises showcasing your favourite Cairo feature
...
The relevant evaluator contract addresses are saved to the `contracts/accounts.json` cache. For devnet testing, the devnet contracts `MUST BE DELETED` every time devnet is restarted. If you would like to disable this contract cache run:

```bash
export ACCOUNT_CACHE=false
```

There were no `action items` for you to complete so you should see a successful `PAYDAY!!!` response from the devnet evaluator contract.
...
- To get the felt representation you can paste the address in this [conversion tool](https://util.turbofish.co).

**_PRIVATE_**

- Select the three vertical dots to display the wallet options
- Select `Export private key`
- Copy the private key from this screen and paste it into `config.json` `TESTNET_ACCOUNT` -> `PRIVATE`.
...
- Copy the public key from this screen and paste it into `config.json` `TESNET_ACCOUNT` -> `PUBLIC`
...
Let's deploy and test the simplest account contract we can, [`hello.cairo`](contracts/hello/hello.cairo):

```bash
python3 contracts/hello/hello.py
```

The job of an account contract is to execute arbitrary business logic on behalf of a specific entity. This is why we see a similar argument pattern for most [execute functions](contracts/hello/hello.cairo#L11).

Follow the prompt and collect 100 points.

### Exercise 2 - [Signatures](./contracts/signature)
...
This leaves the ECDSA signature scheme up to the developer and is typically implemented using the [Pedersen hash](https://docs.starknet.io/documentation/architecture_and_concepts/Hashing/hash-functions/) and native Stark curve:

```bash
python3 contracts/signature/signature_1.py
```

The `signature_1` contract has no concept of a public/private key pair. All the signing was done "off-chain" and yet with account abstraction we're still able to operate a functioning account with a populated signature field.

Follow the prompt and collect 100 points.

#### Signature 2 - 200 pts

Let's couple the signing logic more succinctly with the account:
...
Although we are free to populate the signature field how we please, the StarkNet OS has a specific method for hashing [transaction data](https://docs.starknet.io/documentation/architecture_and_concepts/Blocks/transactions/#invoke_transaction_version_1).

Follow the prompt and collect 200 points.

#### Signature 3 - 300 pts

As StarkNet accounts are simply contracts we can implement any signing mechanism we want. Companies like [Web3Auth](https://medium.com/toruslabs/sign-in-with-starkware-711d48f2dbbd) are using this to create `Sign-In` architectures using your StarkNet account. [JWT](https://github.com/BoBowchan/cairo-jsonwebtoken) token schemes are being implemented.
...
For an example of a unique account architectures we will build a contract that implements its signatures scheme with the `secp256k1` curve and `sha256` instead of our native StarkNet curve:
...
Now that we have implemented the vanilla ECDSA signing mechanisms let's see what account abstraction can really do!

A `multicall` aggregates the results from multiple contract calls. This reduces the number of separate API Client or JSON-RPC requests that need to be sent. In addition, it acts as an `atomic` invocation where all values are returned for the same block.

Popular wallet providers like Argent use this design to implement [account contracts](https://github.com/argentlabs/argent-contracts-starknet/blob/develop/contracts/account/ArgentAccount.cairo) on StarkNet to accommodate a multicall or a single call with one scheme.
...
A `multisig` or multiple signature wallet allows you to share security across multiple signing entities. You can think of them like bank vaults in that they require more than one key to unlock, or in this case, authorize a transaction.
